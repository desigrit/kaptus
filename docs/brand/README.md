...
<img src="kaptus-icon.png" width="160" height="160" alt="An ivory and yellow folded bird inside a teal circle" />

Wordbird is Kaptus's selected brand mark. Its lifted yellow wing carries two caption-shaped openings, while the ivory body and folded tail keep it recognizable as a bird at small sizes.
...
The editable vector master is [`tools/generate_brand_assets.py`](../../tools/generate_brand_assets.py). It preserves the selected concept's yellow folds, ivory body, and upward pose as clean paths and restrained gradients.
...
The bird is shifted 3 dp right from the first vector draft, leaving more air around its raised wingtip. It fits inside Android's central 66 dp safe circle on a 108 dp adaptive layer. Keep this padding when changing the artwork. Kaptus requires Android 13 or newer, so the adaptive resources cover every supported Android version and density.
